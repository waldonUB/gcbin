...
var base64='<table sort-table=\'bdCacheModel.repList\' class="table table-bordered table-striped table-primary" style="width: 297mm;height: auto">\n' +
    '                    <thead>\n' +
    '                    <tr>\n' +
    '                        <th>序号</th>\n' +
    '                        <th>楼栋</th>\n' +
    ...
    '                    </thead>\n' +
    '                    <tbody>\n' +
    '                    <tr ng-repeat="repData in bdCacheModel.repList">\n' +
    '                        <td>{{$index+1}}</td>\n' +
    '                        <td>{{repData.NAME}}</td>\n' +
    '                        <td>{{repData.PRODUCT_NAME}}</td>\n' +
    '                        <td>{{repData.HOUSE_NUM}}</td>\n' +
    '                        <td>{{repData.HOUSE_AREA}}</td>\n' +
    '                        <td>{{repData.HOUSE_PRICE}}</td>\n' +
    '                        <td>{{translateTotal(repData.HOUSE_TOTAL_PRICE)}}</td>\n' +
    '                        <td>{{translateDate(repData.PLAN_PROJECT_PRESALE_DATE)}}</td>\n' +
    '                        <td>{{translateDate(repData.PLAN_CERT_DATE)}}</td>\n' +
    '                        <td>{{translateDate(repData.ACTUAL_CERT_DATE)}}</td>\n' +
    '                        <td>{{repData.UPDATE_USER}}</td>\n' +
    '                        <td>{{repData.UPDATE_DATE}}</td>\n' +
    '                        <td>\n' +
    '                            <i ng-show="repData.APLICATION_STATE==\'10\'">未审核</i>\n' +
    '                            <i ng-show="repData.APLICATION_STATE==\'50\'">已审核</i>\n' +
    '                        </td>\n' +
    '                        <td>\n' +
    '                            <a ng-click="writeBdCache(repData,\'add\')" data-toggle="modal" data-target="#bdModal" class="btn btn-success btn-xs" title="新增">\n' +
    '                                <i class="fa fa-fw fa-plus"></i>\n' +
    '                            </a>\n' +
    '                            <a ng-click="writeBdCache(repData,\'edit\')" data-toggle="modal" data-target="#bdModal" class="btn btn-info btn-xs" title="修改">\n' +
    '                                <i class="fa fa-fw fa-pencil"></i>\n' +
    '                            </a>\n' +
    '                            <a ng-show="repData.ACTUAL_CERT_DATE&&repData.SPLIT_FLAG!=\'是\'" ng-click="synBd(repData)" class="btn btn-success btn-xs" title="同步">\n' +
    '                                <i class="fa fa-fw fa-refresh"></i>\n' +
    '                            </a>\n' +
    '                            <a ng-show="repData.SPLIT_FLAG==\'是\'" ng-click="deleteBdCache(repData)" class="btn btn-danger btn-xs" title="删除">\n' +
